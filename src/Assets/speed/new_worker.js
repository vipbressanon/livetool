...
var settings = {
    test_order: "P_D_U", //order in which tests will be performed as a string. D=Download, U=Upload, P=Ping+Jitter, _=1 second delay
    ...
    time_ulGraceTime: 1.5, //time to wait in seconds before actually measuring ul speed (wait for buffers to fill)
    ...
    url_ping: "ping", // path to an empty file, used for ping test. must be relative to this js file
    ...
    garbagePhp_chunkSize: 25000000, // size of chunks sent by garbage.php (can be different if enable_quirks is active)
...
function dlTest(done) {
    ...
    var downloadSection = [];
    var preSectionLoad = 0;
    var preSectionTime = new Date().getTime();
        ...
        function () {
            ...
            if (!graceTimeDone) {
                if (t > 1000 * settings.time_dlGraceTime) {
                    ...
                    preSectionLoad = totLoaded;
                    preSectionTime = new Date().getTime();
                    graceTimeDone = true;
                }
            } else {
                // 新算法
                var section = {loadData: totLoaded - preSectionLoad, gapTime: new Date().getTime() - preSectionTime};
                section.rate = section.loadData / section.gapTime;
                downloadSection.push(section);
                downloadSection.sort(function (a, b) {
                    return b.rate - a.rate;
                });
                var downloadResultSection;
                if (downloadSection.length > 3) {
                    downloadResultSection = downloadSection.slice(downloadSection.length / 10, downloadSection.length * 2 / 3);
                } else {
                    downloadResultSection = downloadSection;
                }
                var downloadDataSum = 0;
                var downloadTimeSum = 0;
                for (let downloadSecIndex = 0; downloadSecIndex < downloadResultSection.length; downloadSecIndex++) {
                    downloadDataSum += downloadResultSection[downloadSecIndex].loadData;
                    downloadTimeSum += downloadResultSection[downloadSecIndex].gapTime;
                }

                var speed = downloadDataSum / (downloadTimeSum / 1000.0);
                dlStatus = ((speed * 8 * settings.overheadCompensationFactor) / (1024 * 1024)).toFixed(2); // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 or 1000000 to go to megabits/mebibits

                if (((t / 1000.0) > settings.time_dl_max && dlStatus > 0) || failed) { // test is over, stop streams and timer
                    if (failed || isNaN(dlStatus)) dlStatus = 'Fail';
                    clearRequests();
                    clearInterval(interval);
                    dlProgress = 1;
        ...
        interval = setInterval(
            function () {
                ...
                if (!graceTimeDone) {
                    ...
                    var section = {loadData: totLoaded - preSectionLoad, gapTime: new Date().getTime() - preSectionTime};
                    section.rate = section.loadData / section.gapTime;
                    uploadSection.push(section);
                    uploadSection.sort(function (a, b) {
                        return b.rate - a.rate;
                    });
                    var uploadResultSection;
                    if (uploadSection.length > 3) {
                        uploadResultSection = uploadSection.slice(0, uploadSection.length / 2);
                    } else {
                        uploadResultSection = uploadSection;
                    }
                    var uploadDataSum = 0;
                    var uploadTimeSum = 0;
                    for (let uploadSecIndex = 0; uploadSecIndex < uploadResultSection.length; uploadSecIndex++) {
                        uploadDataSum += uploadResultSection[uploadSecIndex].loadData;
                        uploadTimeSum += uploadResultSection[uploadSecIndex].gapTime;
                    }
                    var speed = uploadDataSum / (uploadTimeSum / 1000.0);
                    ulStatus = ((speed * 8 * settings.overheadCompensationFactor) / (1024 * 1024)).toFixed(2); // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 or 1000000 to go to megabits/mebibits

                    if (((t / 1000.0) > settings.time_ul_max && ulStatus > 0) || failed) { // test is over, stop streams and timer
                        if (failed || isNaN(ulStatus)) ulStatus = 'Fail';
                        clearRequests();
                        clearInterval(interval);
                        ulProgress = 1;
...
function pingTest(done) {
    ...
    //             doPing();
    //         } else {
    //             if (lossStatus === "") lossStatus = 0; //无丢包数据时，默认显示为0
    //             // more pings to do?
    //             pingProgress = 1;
    ...
    //             done();
    //         }
    //         if (settings.xhr_ignoreErrors === 1) {//retry ping
    //             lossNum++;
    //             lossStatus = parseInt((lossNum / settings.count_ping) * 100);
    //             doPing();
    //         }
    ...
    var prevInstspd = 0; // last ping time, used for jitter calculation
    xhr = [];
    var jitterRecords = [];  // jitter result record
    // ping function
    var doPing = function () {
        tlog('ping');
        pingProgress = i / settings.count_ping;
        ...
        xhr[0].onload = function () {
            ...
            if (i === 0) {
                ...
                if (i > 1) {
                    var instjitter = Math.abs(instspd - prevInstspd);
                    jitterRecords.push(instjitter);
                    var jitterSum = 0;
                    for (var jitterForIndex = 0; jitterForIndex < jitterRecords.length; jitterForIndex++) {
                        jitterSum += jitterRecords[jitterForIndex];
                    }
                    jitter = jitterSum / jitterRecords.length;
                }
                //
                ...
                //     testLog("ping -> " + ping + ", instspd -> " + instspd + ", jitter -> " + jitter);
                // }
                prevInstspd = instspd;
            }
        ...
        //设置超时，避免第一个节点不能用造成整个测速都不能进行
        try {
            xhr[0].timeout = 500; //设置超时
