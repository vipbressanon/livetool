...
class MsgTest extends Command
{
    ...
    private static $onoffinit = ['roomtype'=>2, 'ischat'=>1, 'ishand'=>1, 'max'=>'', 'boardscale' => 100, 'share' => '', 'practice' => ['status' => 0, 'scratch_id' => '', 'course_hash_id' => '', 'chapter_hash_id' => '', 'class_hash_id' => '']];
            ...
            $socket->on('im', function ($request) use ($socket)  {
                try {
                    ...
                    if ($request['type'] == 'MAX') {
                        ...
                        // 编程的练习模式 将数据存入onoff 初始化教室数据可以取到
                        /*$arrRoomPractice['status'] = isset($request['status']) ? $request['status'] : '';
